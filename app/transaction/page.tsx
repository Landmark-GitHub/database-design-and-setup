...
export default function LedgerPage() {
  ...
  const handleExportCSV = () => {
    if (recordSummaryByType.length === 0) {
      alert('ไม่มีข้อมูลสรุปยอดของวันนี้ให้กด Export');
      return;
    }

    // 1. หัวตารางกำหนดคอลัมน์ตามที่ระบุ
    const headers = [
      'Date',
      'Member',
      'Product',
    ...
    // 2. แปลงชุดข้อมูลสรุปตาม ชนิดสินค้า (Type) มารวมฟิลด์บิลหลัก
    const rows = recordSummaryByType.map((summaryItem) => {
      // ค้นหาบิลหลักใบที่ตรงกันเพื่อดึงยอด น้ำแข็งแห้ง (D) ยอดรวม และสถานะจ่ายเงิน
      const matchedBill = filteredBills.find(
        (b) => b.memberId === summaryItem.memberId && b.date === summaryItem.date
      );

      // คำนวณข้อมูลงวดน้ำแข็งแห้ง (Type "D") เฉพาะของสมาชิกคนนี้ในวันนี้
      let iceQty = 0;
      let icePriceTotal = 0;

      if (matchedBill) {
        const iceProducts = allProducts.filter((p) => p.type === 'D');
        const iceItems = matchedBill.items.filter((item) => iceProducts.some((p) => p.id === item.productId));
        
        // ยอดเบิกรวมน้ำแข็งแห้ง (New) ของบิลใบนี้
        iceQty = iceItems.reduce((sum, i) => sum + (i.newStock || 0), 0);

        // คำนวณราคาของน้ำแข็งแห้งตาม Class สมาชิก
        const member = members.find((m) => m.id === matchedBill.memberId);
        const memberClass = member?.class ?? 'In';
        const baseIce = iceProducts[0];
        let icePriceUnit = 0;
        if (memberClass === 'Out') icePriceUnit = baseIce?.priceOut || 0;
        else if (memberClass === 'WalkIn') icePriceUnit = baseIce?.priceWorkIn || 0;
        else icePriceUnit = baseIce?.priceIn || 0;

        icePriceTotal = iceQty * icePriceUnit;
      }

      const totalSales = matchedBill ? Math.round(matchedBill.totalSales) : 0;
      const amountPaid = matchedBill ? Math.round(matchedBill.amountPaid) : 0;
      const amountOwed = matchedBill ? Math.round(matchedBill.amountOwed) : 0;
      
      // ฟอร์แมตส่วนแสดงผล ชำระแล้ว/ค้างชำระ
      const paidOwedText = `จ่ายแล้ว:${amountPaid}/ค้าง:${amountOwed}`;

      return [
        summaryItem.date,                                     // Date
        `"${summaryItem.memberName.replace(/"/g, '""')}"`,     // Member
        `"${summaryItem.typeName.replace(/"/g, '""')}"`,       // Product (ในที่นี้คือชื่อชนิดกลุ่มไอศกรีม)
        summaryItem.oldStock,                                 // Old
        summaryItem.newStock,                                 // New
        summaryItem.totalStock,                               // Total
        summaryItem.returned,                                 // Returned
        summaryItem.sold,                                     // SoldPrice (ยอดที่ขายได้กี่ชิ้น)
        Math.round(summaryItem.netPrice),                     // Subtotal (ยอดเงินของสินค้าประเภทนี้)
        iceQty,                                               // Ice (จำนวนเบิกน้ำแข็งแห้ง)
        icePriceTotal,                                        // Ice Price (ราคาน้ำแข็งแห้งรวม)
        totalSales,                                           // Total Sales (ยอดรวมทั้งหมดทั้งบิล)
        `"${paidOwedText}"`,                                  // PaidOwed
        `"${(matchedBill?.status || 'draft').toUpperCase()}"`  // Notes (ใส่สถานะของบิลไว้เบื้องต้น)
      ];
    });

    // 3. รวม Header และข้อมูล เข้าด้วยกันสลับด้วยเครื่องหมายจุลภาค (Comma)
    const csvContent = [
      headers.join(','),
      ...rows.map((row) => row.join(','))
    ].join('\n');

    // 4. สั่งสร้างไฟล์ดาวน์โหลดและใส่ BOM (\uFEFF) ป้องกัน Excel อ่านภาษาไทยเพี้ยน
    ...
    link.href = url;
    link.setAttribute('download', `Daily_Summary_Type_${selectedDate}.csv`);
    ...
    <main className="min-h-screen bg-background p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-card p-4 rounded-2xl border shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* ปุ่มดาวน์โหลด CSV สรุปชนิดยอดสินค้าตามที่คุณขอ */}
            <button
              onClick={handleExportCSV}
              className="inline-flex items-center justify-center px-4 h-10 text-sm font-bold text-white bg-emerald-600 rounded-xl hover:bg-emerald-700 active:scale-95 transition-all shadow-sm gap-1 self-end"
            >
              📥 ดาวน์โหลด CSV
            </button>
